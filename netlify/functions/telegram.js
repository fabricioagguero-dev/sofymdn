...
async function routeTextMessage(chatId, session, message) {
  ...
  console.log(`[Router] chat: ${chatId} | Texto "${text}" | Foto: ${hasphoto} | Estado: ${session.step}`);

  // 1. DETECTOR DE FOTOS (Prioridad Máxima)
  if (hasphoto) {
    if (session.step === "idle") {
      await manejarSubidaFoto(chatId, session, message.photo);
      return;
    } else {
      // Agregado el chatId que faltaba aquí abajo
      await sendMd(chatId, "⚠️ *Ya se está procesando una prenda\\.*\nTerminá el formulario o usá /cancelar\\.");
      return;
    }
  }

  // 2. COMANDOS (Solo si no fue una foto)
  ...
  if (text === "/cancelar" || text === "/cancel" || text === "❌ Cancelar operación") {
    ...
    await sendMd(
      ...
      haySesion
        ...
        : "No había ninguna operación en curso\\.",
      { reply_markup: KB_REMOVE }
    );
    return;
  }
  ...
  if (text === "/subir") {
    if (session.step !== "idle") {
      await sendMd(chatId, "⚠️ Ya tengo una operación en curso\\.");
      return;
    }
    await sendMd(chatId, "📸 *Subir prenda\\(s\\)*\n\nMandame la foto de la prenda\\.", { reply_markup: KB_WITH_CANCEL });
    return;
  }
  ...
  if (text === "/eliminar") {
    if (session.step !== "idle") {
      await sendMd(chatId, "⚠️ Usá /cancelar primero\\.");
      return;
    }
  ...
  // 3. ESTADOS DEL FLUJO (Solo si no es comando ni foto nueva)
  if (session.step === "wait_album_answer") {
    if (text === "🎨 Sí, mismo modelo — distintos colores") {
      session.mode = "album";
      session.step = "asking_product";
      session.current = 0;
      await sendMd(chatId, `🎨 *Perfecto\\!* Datos una sola vez\\.`, { reply_markup: KB_REMOVE });
      await new Promise(r => setTimeout(r, 100)); // Bajé el tiempo para Netlify
      await iniciarPreguntasPrenda(chatId, session, 0, false);
      ...
      session.current = 0;
      const count = session.queue.length;
      await sendMd(chatId, `📦 *Entendido\\!* Tenés *${count} fotos*\\.`, {
        reply_markup: {
          inline_keyboard: [[
      ...
      await sendPlain(chatId, "👆 Por favor usá los botones de arriba.");
    }
    return;
  }
...
if (session.step === "asking_product") {
  ...
  // 4. FALLBACK (Si nada de lo anterior coincidió)
  if (session.step !== "idle") {
    await sendMd(chatId, "⚠️ *No entendí esa respuesta\\.*\nPor favor respondé lo que te pregunto o usá /cancelar\\.");
  }
  
} // <--- ESTA ES LA LLAVE QUE TE FALTA. Agregala aquí.
