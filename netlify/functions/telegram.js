...
/**
 * Enruta todos los mensajes de texto recibidos según el estado de la sesión.
 * Funciona como un controlador principal de la conversación.
 *
 * @param {string|number} chatId
 * @param {Object}        session
...
async function routeTextMessage(chatId, session, message) {
  ...
  // ── COMANDOS (siempre tienen prioridad sobre cualquier estado) ──────────────
  ...
  if (text === "/cancelar" || text === "/cancel" || text === "❌ Cancelar operación") {
    const haySesion = session.step !== "idle";
    resetSession(chatId);
    await sendMd(
      chatId,
      haySesion
        ? "❌ *Operación cancelada\\.* Todo vuelve a cero\\.\n\nUsá /start para ver los comandos\\."
        : "No había ninguna operación en curso\\. Usá /start para comenzar\\.",
      { reply_markup: KB_REMOVE }
    );
    return;
  }
  ...
  if (text === "/subir") {
    if (session.step !== "idle") {
      await sendMd(
        chatId,
        "⚠️ Ya tengo una operación en curso\\. Usá /cancelar primero si querés empezar de nuevo\\."
      );
      return;
    }
    await sendMd(
      chatId,
      "📸 *Subir prenda\\(s\\)*\n\nMandame la foto o fotos de la\\(s\\) prenda\\(s\\) que querés subir\\.\n\n_Si querés subir varios colores del mismo modelo, mandá todas las fotos juntas\\._",
      { reply_markup: KB_WITH_CANCEL }
    );
    return;
  }
  ...
  if (text === "/eliminar") {
    if (session.step !== "idle") {
      await sendMd(chatId, "⚠️ Usá /cancelar primero para terminar lo que estabas haciendo\\.");
      return;
    }
  ...
  // Estado: esperando respuesta sobre si es álbum o prendas distintas
  if (session.step === "wait_album_answer") {
    if (text === "🎨 Sí, mismo modelo — distintos colores") {
      ...
      await sendMd(
        chatId,
        `🎨 *Perfecto\\!*\nVoy a preguntarte los datos *una sola vez* y todas las fotos quedarán como colores de esa prenda\\.`,
        { reply_markup: KB_REMOVE }
      );
      ...
      await sendMd(
        chatId,
        `📦 *Entendido\\!* Voy a preguntarte los datos de cada prenda por separado\\.\n\nTenés *${count} fotos*\\. Las voy a numerar del 1 al ${count} según el orden en que me las enviaste\\.\n\n¿Querés que te explique cómo funciona antes de arrancar?`,
        {
          reply_markup: {
            inline_keyboard: [[
              { text: "📖 Sí, explicame", callback_data: "explain__yes" },
              { text: "▶️ No, arrancamos", callback_data: "explain__no"  },
            ]],
          },
        }
      );
    } else {
      await sendPlain(chatId, "👆 Por favor usá los botones de arriba para responder.");
    }
    return;
  }

  // Estado: esperando ID para eliminar
  if (session.step === "wait_delete_id") {
    await procesarIdEliminacion(chatId, session, text);
    return;
  }

  // Estado: en medio de preguntas de prenda
  if (session.step === "asking_product") {
    await procesarRespuestaPrenda(chatId, session, text);
    return;
  }

  // Estado: esperando confirmación de eliminación (se maneja por callback, pero por si acaso)
  if (session.step === "confirm_delete") {
    await sendPlain(chatId, "👆 Por favor usá los botones para confirmar o cancelar.");
    return;
  }

  // ── IDLE: sin contexto de conversación ───────────────────────────────────────
  if (session.step === "idle") {
    if (text.startsWith("/")) {
      await sendMd(
        chatId,
        `❓ No conozco ese comando\\. Usá /ayuda para ver todos los comandos disponibles\\.`
      );
    } else {
      await sendMd(
        chatId,
        `👋 ¡Hola\\! Usá /start para ver el menú o mandame una foto para subir una prenda\\.`
      );
    }
    return;
  }
  ...
  console.warn(`[router] Estado desconocido: ${session.step}, texto: ${text}`);
  await sendMd(
    chatId,
    `_Algo salió mal\\. Usá /cancelar para reiniciar\\._`
  );
}
