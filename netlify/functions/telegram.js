...
//   IMGBB_API_KEY        → API Key de imgbb.com  (imágenes permanentes)
//
...
//
// FLUJO DE CARGA:
//   1 foto → Tela → Talles → Precio → Título → Descripción → GitHub
//   Varias fotos → ¿Mismo modelo? →
//     Sí: picker de color por foto → datos del modelo → GitHub
//     No: pregunta cada prenda por separado
//
// NOTA SOBRE MULTI-FOTO EN NETLIFY:
//   Las funciones serverless no comparten memoria entre invocaciones.
//   Para acumular las fotos de un álbum se usa un archivo temporal en
//   GitHub (data/_pending.json). Cada foto escribe su fileId ahí;
//   la primera invocación en "reclamar" el grupo lo procesa.
//
// =============================================================================
...
// Archivo temporal para acumular fotos de álbumes (multi-foto)
const GITHUB_PENDING = "data/_pending.json";
...
//     Para la acumulación de álbumes (multi-foto) se usa GitHub como store
//     compartido — ver Sección 4 (Pending Groups).
...
function newSession() {
  return {
    ...
    // wait_album_answer | wait_color_pick
...
async function logChange(accion, detalle) {
  try {
    ...
    entries.unshift({ fecha: new Date().toISOString(), accion, detalle });
...
// ─────────────────────────────────────────────────────────────────────────────
// SECCIÓN 4 — PENDING GROUPS (acumulación de álbumes en GitHub)
// ─────────────────────────────────────────────────────────────────────────────
//
// Solución al problema de instancias separadas en Netlify Functions:
// Cada foto de un media_group escribe en data/_pending.json.
// La primera invocación que logra "reclamar" el grupo lo procesa.
// Los grupos con más de 5 minutos se limpian automáticamente.

async function readPending() {
  try {
    const { status, data } = await ghReq("GET", GITHUB_PENDING);
    if (status === 404) return { groups: {}, sha: null };
    if (status !== 200 || !data?.content) return { groups: {}, sha: null };
    const raw = Buffer.from(data.content, "base64").toString("utf8").trim();
    const parsed = raw ? JSON.parse(raw) : {};
    return { groups: parsed, sha: data.sha };
  } catch {
    return { groups: {}, sha: null };
  }
}

async function writePending(groups, sha) {
  try {
    // Limpiar grupos viejos (>5 min) antes de escribir
    const now = Date.now();
    for (const key of Object.keys(groups)) {
      if (now - (groups[key].createdAt || 0) > 5 * 60 * 1000) {
        delete groups[key];
      }
    }
    const content = Buffer.from(JSON.stringify(groups)).toString("base64");
    const { status } = await ghReq("PUT", GITHUB_PENDING, {
      message: "pending groups update",
      content,
      ...(sha ? { sha } : {}),
    });
...
// Agrega una foto al grupo pendiente. Reintenta hasta 3 veces si hay conflicto de SHA.
async function addPhotoToGroup(chatId, mgId, photo) {
  for (let attempt = 0; attempt < 3; attempt++) {
    const { groups, sha } = await readPending();
    if (!groups[mgId]) {
      groups[mgId] = { chatId, photos: [], createdAt: Date.now(), claimed: false };
    }
    // Evitar duplicados (si la misma invocación reintenta)
    const exists = groups[mgId].photos.some(p => p.fileId === photo.fileId);
    if (!exists) groups[mgId].photos.push(photo);

    const ok = await writePending(groups, sha);
    if (ok) return groups[mgId].photos.length;

    // Conflicto de SHA (otra invocación escribió primero): reintentar
    await new Promise(r => setTimeout(r, 150 * (attempt + 1)));
  }
  return 0;
}

// Intenta "reclamar" el grupo para procesarlo. Devuelve las fotos si lo logra, null si otro ya lo reclamó.
async function claimGroup(mgId) {
  for (let attempt = 0; attempt < 3; attempt++) {
    const { groups, sha } = await readPending();
    const group = groups[mgId];
    if (!group || group.claimed) return null; // otro ya lo reclamó

    group.claimed = true;
    const photos = [...group.photos];
    const ok = await writePending(groups, sha);
    if (ok) return photos; // éxito: devolvemos las fotos acumuladas
...
// Limpia el grupo del archivo pendiente después de procesarlo
async function cleanupGroup(mgId) {
  try {
    const { groups, sha } = await readPending();
    delete groups[mgId];
    await writePending(groups, sha);
  } catch {}
}

// ─────────────────────────────────────────────────────────────────────────────
// SECCIÓN 5 — TELEGRAM API
// ─────────────────────────────────────────────────────────────────────────────
...
// Obtener la URL temporal de descarga de un archivo de Telegram
...
// ─────────────────────────────────────────────────────────────────────────────
// SECCIÓN 6 — IMGBB (almacenamiento permanente de imágenes)
// ─────────────────────────────────────────────────────────────────────────────
//
// Las URLs de Telegram expiran en ~24 hs. Esta función descarga la imagen
// y la sube a imgbb para obtener una URL permanente.
// Si IMGBB_API_KEY no está configurada, cae en la URL temporal de Telegram.
// Obtené tu key gratis en: https://api.imgbb.com/

// Descargar imagen de Telegram como Buffer
function downloadFromTelegram(url) {
  return new Promise(resolve => {
...
// Subir imagen a imgbb y devolver URL permanente
async function uploadToImgbb(fileId) {
  // Obtener URL temporal de Telegram
  const tgUrl = await getFileUrl(fileId);
  if (!tgUrl) return "";

  // Si no hay key de imgbb, devolver la URL temporal (expirará en ~24hs)
  if (!IMGBB_KEY) {
    console.warn("[imgbb] IMGBB_API_KEY no configurada — usando URL temporal de Telegram");
    return tgUrl;
  }

  try {
    // Descargar la imagen
    const buffer = await downloadFromTelegram(tgUrl);
    if (!buffer || buffer.length === 0) return tgUrl;

    const base64 = buffer.toString("base64");
    const body   = `key=${encodeURIComponent(IMGBB_KEY)}&image=${encodeURIComponent(base64)}`;
        ...
        res.on("end", () => {
          try {
            ...
            if (d.success && d.data?.url) {
              console.log("[imgbb] ✅ Subida exitosa:", d.data.url);
              resolve(d.data.url);
            } else {
              console.error("[imgbb] Error:", d.error?.message || "respuesta inválida");
              resolve(tgUrl); // fallback
            }
          } catch { resolve(tgUrl); }
        });
      });
      req.on("error",   () => resolve(tgUrl));
      req.on("timeout", () => { req.destroy(); resolve(tgUrl); });
      req.write(body);
      req.end();
    });
    ...
    return tgUrl; // fallback a URL temporal
  }
}
...
// Picker manual de colores
async function sendColorPicker(chatId) {
  const rows = [];
  for (let i = 0; i < COLORES.length; i += 3) rows.push(COLORES.slice(i, i+3));
  await sendBtns(chatId, "🎨 ¿De qué color es? Elegí o escribilo:", rows);
}

// ─────────────────────────────────────────────────────────────────────────────
// SECCIÓN 9 — FLUJO: RECIBIR FOTOS
// ─────────────────────────────────────────────────────────────────────────────
//
// Foto suelta → flujo normal
// Álbum (media_group) → acumular en GitHub → preguntar modo
...
async function handlePhoto(chatId, session, message) {
  ...
  // ── Álbum (media_group): acumular fotos en GitHub ─────────────────────────
  //
  // No podemos confiar en la memoria (Netlify crea una instancia separada por
  // request). Usamos data/_pending.json como store compartido.

  await addPhotoToGroup(chatId, mgId, { fileId, width, height });

  // Solo el primer mensaje que llega para este grupo envía el aviso
  if (session.step === "idle") {
  ...
  // Esperamos a que lleguen todas las fotos del grupo
  await new Promise(r => setTimeout(r, MEDIA_WAIT));

  // Intentar reclamar el grupo (solo una instancia lo logra)
  const accumulatedPhotos = await claimGroup(mgId);
  if (!accumulatedPhotos) {
    // Otro invocation ya reclamó el grupo, esta instancia no hace nada más
    return;
  }

  // Tenemos todas las fotos → continuar el flujo
  accumulatedPhotos.sort((a, b) => b.height - a.height);
  session.photos     = accumulatedPhotos;
  session.mediaGroup = mgId;

  await sendBtns(
    chatId,
    `📸 Recibí *${accumulatedPhotos.length} fotos*.\n\n¿Son del mismo modelo en distintos colores, o son prendas distintas?`,
    [["🎨 Mismo modelo, distintos colores"], ["📦 Son prendas distintas"]],
    { parse_mode: "Markdown" }
  );
  session.step = "wait_album_answer";

  // Limpiar el pending group en segundo plano
  cleanupGroup(mgId).catch(() => {});
}

// ─────────────────────────────────────────────────────────────────────────────
// SECCIÓN 10 — FLUJO: ÁLBUM (mismo modelo, distintos colores)
// ─────────────────────────────────────────────────────────────────────────────
...
async function processNextColor(chatId, session) {
  ...
  await sendMsg(chatId,
    `🎨 *Foto ${idx+1} de ${total}* — ¿De qué color es esta prenda?`,
    { parse_mode: "Markdown" }
  );
...
// ─────────────────────────────────────────────────────────────────────────────
// SECCIÓN 11 — FLUJO: INDIVIDUAL (prendas distintas)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// SECCIÓN 12 — GUARDAR PRODUCTOS EN GITHUB (con imgbb)
// ─────────────────────────────────────────────────────────────────────────────

async function saveProduct(chatId, session) {
  await sendMsg(chatId, "⏳ Subiendo imágenes y guardando...");
  try {
    const { products, sha } = await readDB();
    let producto;

    if (session.mode === "album") {
      // Álbum: subir cada foto a imgbb en paralelo
      await sendMsg(chatId, "🖼️ Subiendo fotos a imgbb...");
      const imagenes = (await Promise.all(
        session.albumColors.map(c => uploadToImgbb(c.fileId))
      )).filter(Boolean);
      ...
      // Prenda única: subir foto a imgbb
      const url = await uploadToImgbb(session.photos[0].fileId);
      producto = {
        id: genId(), nombre: session.titulo, tela: session.tela,
        talles: session.talles, precio: session.precio,
        descripcion: session.descripcion || "", tipo: "otro",
...
async function saveIndivProduct(chatId, session) {
  await sendMsg(chatId, "⏳ Subiendo imagen y guardando...");
  try {
    const { products, sha } = await readDB();
    const d   = session.indivData;
    const url = await uploadToImgbb(session.photos[session.indivIdx].fileId);
...
// ─────────────────────────────────────────────────────────────────────────────
// SECCIÓN 19 — COMANDO: /precios (actualización masiva)
// ─────────────────────────────────────────────────────────────────────────────
...
async function handleText(chatId, session, text) {
  ...
  if (text === "/ayuda") {
    await sendMd(chatId,
      ...
      `📸📸 *Varias fotos juntas* →\n` +
      `  🎨 Mismo modelo → El bot te pide el color de cada foto con un picker\n` +
  ...
  // Color escrito a mano (cuando el usuario escribe un color que no está en el picker)
  if (session.step === "wait_color_pick") {
...
// ─────────────────────────────────────────────────────────────────────────────
// SECCIÓN 22 — ROUTER DE CALLBACKS
// ─────────────────────────────────────────────────────────────────────────────
...
async function handleCallback(chatId, session, cbId, data) {
  ...
  // ── Color picker (picker manual, sin IA) ────────────────────────────────────
  if (session.step === "wait_color_pick" && COLORES.includes(data)) {
  ...
  if (data.startsWith("editcampo__")) {
    ...
    if (campo === "tela") {
      ...
      await sendBtns(chatId, "🏷️ Elegí el nuevo tipo:", [TIPOS.slice(0,4), TIPOS.slice(4)]);
    } else {
...
// ─────────────────────────────────────────────────────────────────────────────
// SECCIÓN 23 — ENTRY POINT (Netlify Function Handler)
// ─────────────────────────────────────────────────────────────────────────────
